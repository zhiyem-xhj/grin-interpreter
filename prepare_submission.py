...
def _has_no_committed_changes(
        git_executable_path: Path,
        working_directory_path: Path) -> bool:
    """Returns True if there are no committed changes in the working directory
    (i.e., the Git repository has no commits after its creation)."""

    # Check if HEAD has a parent.  If not, it's reasonable to assume that there
    # are no changes that have been committed in this repository since it was
    # first created.
    exit_code, _ = _execute_git(
        git_executable_path, working_directory_path,
        ['rev-list', 'HEAD~1'],
        print_error_output = False)
...
def main() -> None:
    ...
    if _has_no_committed_changes(git_executable_path, working_directory_path):
        print('Nothing has been committed to your Git repository since it was first created.')
        print('This strongly suggests that what you\'ll be submitting will be only the code')
        print('that we\'ve provided, which means YOUR SCORE WILL BE ZERO if you submit it.')
